...
open class GSImageViewerController: UIViewController, UIImagePickerControllerDelegate {
...
//        let button = UIButton()
//        button.frame = CGRect(x: 0, y: self.view.frame.size.height, width: self.view.frame.size.width, height: 100)
//        button.setTitle("Save to camera roll", for: UIControlState.normal)
//        button.titleLabel?.font = UIFont.systemFont(ofSize: 20)
//        button.titleLabel?.textColor = UIColor.black
//        scrollView.addSubview(button)
//        scrollView.bringSubview(toFront: button)
//        button.addTarget(self, action: #selector(saveImage), for: UIControlEvents.touchUpInside)
        
        let width = self.view.frame.size.width
        let height = self.view.frame.size.height
        
        let button = UIButton()
        button.setTitle("SAVE WALLPAPER", for: UIControlState.normal)
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 15)
        button.setTitleColor(UIColor.darkGray, for: .normal)
        button.backgroundColor = UIColor.white
        button.layer.cornerRadius = 27
        button.frame = CGRect(x: width/4, y: height+120, width: width/2, height: 54)
        ...
        UIView.animate(withDuration: 0.2, animations: {
            button.frame = CGRect(x: width/4, y: self.view.frame.size.height - 110, width: button.frame.size.width, height: 54)
        }) { (success) in
            UIView.animate(withDuration: 0.15, animations: {
                button.frame = CGRect(x: width/4, y: self.view.frame.size.height - 100, width: button.frame.size.width, height: 54)
            }) { (success) in
